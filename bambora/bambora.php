...
class Bambora extends PaymentModule {
	...
	public function getContent() {
		...
		if ( Tools::isSubmit( 'submit' . $this->name ) ) {
			...
			if ( empty( $merchantnumber ) || ! Validate::isGenericName( $merchantnumber ) ) {
				...
				Configuration::updateValue( 'BAMBORA_TERMS_URL', Tools::getValue( "BAMBORA_TERMS_URL" ) );
				$output .= $this->displayConfirmation( $this->l( 'Settings updated' ) );
			}
		}
		if ( ! $this->testCredentials() ) {
			$output .= $this->displayError( 'The credentials you have given are not valid. Please check them.' );
		}
	...
	private function displayForm() {
		...
		$helper->fields_value['BAMBORA_TERMS_URL']                       = Configuration::get( 'BAMBORA_TERMS_URL' );
		$html                                                            = '<div class="row">
	...
	public function hookDisplayHeader() {
		if ( $this->context->controller != null ) {
			...
			if ( $this->getPsVersion() === $this::V17 ) {
				...
				$this->context->controller->registerJavascript( 'checkout-sdk-web.min', $jsUrl, [
					'position' => 'head',
					'server'   => 'remote'
				] );
			} else {
	...
	public function createCheckoutRequest( $cart ) {
		...
		$request->instantcaptureamount = Configuration::get( 'BAMBORA_INSTANTCAPTURE' ) == 1 ? $bamboraOrder->amount : 0;
		$request->order                = $bamboraOrder;
		$request->url                  = $bamboraUrl;
		if ( Configuration::get( 'BAMBORA_ALLOW_LOW_VALUE_EXEMPTION' ) ) {
			if ( $request->order->amount < BamboraCurrency::convertPriceToMinorUnits( Configuration::get( 'BAMBORA_LIMIT_FOR_LOW_VALUE_EXEMPTION' ), BamboraCurrency::getCurrencyMinorunits( $request->order->currency ), Configuration::get( 'BAMBORA_ROUNDING_MODE' ) ) ) {
		...
		$request->paymentwindow->id       = is_numeric( $paymentWindowId ) ? $paymentWindowId : 1;
	...
	private function createBamboraOrder( $cart, $invoiceAddress, $deliveryAddress ) {
		...
		$bamboraOrder->amount = BamboraCurrency::convertPriceToMinorUnits( $cart->getOrderTotal(), $minorUnits, Configuration::get( 'BAMBORA_ROUNDING_MODE' ) );
	...
	private function createBamboraUrl( $isPaymentRequest = false ) {
		...
		if ( $isPaymentRequest ) {
			$callback->url = $this->context->link->getModuleLink( $this->name, 'paymentrequestcallback', array(), true );
		} else {
				...
				if ( ! $containPaymentWithTransactionId && $hasPaymentRequestPermissions ) {
					if ( Tools::isSubmit( 'createpaymentrequest' ) ) {
						if ( ! $this->getDbPaymentRequestByOrderId( $order->id ) ) {
							...
							$html = $this->displayError( "There is already a payment request for this order." );
						}
					}
		...
		try {
			...
			if ( $this->isPsVersionHigherThan177() ) {
				$html .= '<div class="card-body">
                        ...
                        <div>'
				         . $this->buildPaymentTable( $transactionInfo, $iso_code, $transactionOperations )
				         . $this->buildButtonsForm( $transactionInfo )
				         . '</div>

                        <div class="">'
				         . $this->createCheckoutTransactionOperationsHtml( $transactionOperations, $iso_code )
				         . '</div>
                      </div>'
				         . '</div>';
			} else {
	...
	private function buildPaymentTable( $transactionInfo, $currency, $transactionOperations ) {
		...
		$formattedAmount = $this->context->currentLocale->formatPrice( $amount, $currency );
		...
		foreach ( $transactionOperations as $operation ) {
			...
			if ( $eventInfo['description'] != null ) {
				...
				if ( $operation["amount"] > 0 ) {
					...
					$res    .= '<td>' . $this->context->currentLocale->formatPrice( $amount, $currency ) . '</td>';
				} else {
					$res .= '<td> &nbsp; </td>';
				}
	...
	private function getDbPaymentRequestByOrderId( $id_order ) {
		$query = 'SELECT * FROM ' . _DB_PREFIX_ . 'bambora_payment_requests WHERE id_order = ' . pSQL( $id_order );
	...
	private function getDbPaymentRequests( $query ) {
		...
		if ( ! isset( $paymentRequests ) || count( $paymentRequests ) === 0 || ! isset( $paymentRequests[0]['payment_request_id'] ) ) {
			return false;
		}
	...
	private function createPaymentRequest( $order ) {
		...
		try {
			...
			$paymentRequest->description = Tools::getValue( 'bambora_paymentrequest_requester_description' );
			...
			$paymentRequestParameters->instantcaptureamount = Configuration::get( 'BAMBORA_INSTANTCAPTURE' ) == 1 ? $bamboraOrder->amount : 0;
			...
			$paymentRequestParameters->paymentwindow->id       = is_numeric( $paymentWindowId ) ? $paymentWindowId : 1;
			$paymentRequestParameters->paymentwindow->language = str_replace( "_", "-", $language->locale );
			...
			if ( $createPaymentRequest['meta']['result'] ) {
				$message = $this->l( 'Payment request created with ID:' ) . $createPaymentRequest['id'];
				$msg     = new Message();
				$this->addDbPaymentRequest( $order->id, $cart->id, $createPaymentRequest['id'], $createPaymentRequest['url'] );
	 ...
	 * @return bool
	 */
	public function addDbPaymentRequest( $id_order, $id_cart, $payment_request_id, $url ) {


		$query = 'INSERT INTO ' . _DB_PREFIX_ . 'bambora_payment_requests
                (id_order, id_cart, payment_request_id, payment_request_url, date_add)
                VALUES
                (' . pSQL( $id_order ) . ', ' . pSQL( $id_cart ) . ', \'' . pSQL( $payment_request_id ) . '\',  \'' . pSQL( $url ) . '\', NOW() )';
	...
	private function deletePaymentRequest( $order ) {
		...
		try {
			...
			$apiKey               = $this->getApiKey();
			$api                  = new BamboraApi( $apiKey );
			$deletePaymentRequest = $api->deletePaymentRequest( $paymentRequest['payment_request_id'] );

			if ( $deletePaymentRequest['meta']['result'] ) {
				$message = $this->l( 'Payment request deleted. ID:' ) . $paymentRequest['payment_request_id'];
				$msg     = new Message();
	...
	public function deleteDbPaymentRequest( $payment_request_id ) {
		...
		$query = 'DELETE FROM ' . _DB_PREFIX_ . 'bambora_payment_requests WHERE payment_request_id="' . pSQL( $payment_request_id ) . '"';
	...
	private function sendPaymentRequest( $order ) {
		...
		try {
			...
			$recipient->replyto->name  = Tools::getValue( 'bambora_paymentrequest_replyto_name' );
			$recipient->replyto->email = Tools::getValue( 'bambora_paymentrequest_replyto_email' );
			$recipient->message        = Tools::getValue( 'bambora_paymentrequest_email_message' );
			$recipient->to             = new BamboraCheckoutPaymentRequestEmailRecipientAddress();
			$recipient->to->email      = Tools::getValue( 'bambora_paymentrequest_recipient_email' );
			$recipient->to->name       = Tools::getValue( 'bambora_paymentrequest_recipient_name' );
			$jsonData                  = json_encode( $recipient );
			$apiKey                    = $this->getApiKey();
			$api                       = new BamboraApi( $apiKey );
			$sendPaymentRequestEmail   = $api->sendPaymentRequestEmail( $paymentRequest['payment_request_id'], $jsonData );
	...
	private function displayPaymentRequestForm( $params ) {
		...
		// Load current value
		$helper->fields_value['bambora_paymentrequest_replyto_name'] = Tools::getValue( 'bambora_paymentrequest_replyto_name' ) ? Tools::getValue( 'bambora_paymentrequest_replyto_name' ) : $employee->firstname . ' ' . $employee->lastname;;
		$helper->fields_value['bambora_paymentrequest_email_message'] = Tools::getValue( 'bambora_paymentrequest_email_message' );
		...
		$helper->fields_value['bambora_paymentrequest_amount'] = number_format( $order->total_paid, 2, '.', '' );
		...
		if ( $paymentRequest ) {
			$html .= '<b>' . $this->l( 'Payment Request ID' ) . ':</b> ' . $paymentRequest['payment_request_id'] . '</a><br/>';


			$apiKey                = $this->getApiKey();
			$api                   = new BamboraApi( $apiKey );
			$paymentRequestDetails = $api->getPaymentRequest( $paymentRequest['payment_request_id'] );
			$html                  .= "<b>" . $this->l( 'Created' ) . ":</b> " . Tools::formatDateStr( $paymentRequestDetails['createddate'] ) . "<br/>";
			$html                  .= "<b>" . $this->l( 'Description' ) . ":</b> " . $paymentRequestDetails['description'] . "<br/>";
			$html                  .= "<b>" . $this->l( 'Status', 'bambora' ) . ":</b> " . $paymentRequestDetails['status'] . "<br/>";
			$html                  .= "<b>" . $this->l( 'Reference' ) . ":</b> " . $paymentRequestDetails['reference'] . "<br/>";

			$currency = $paymentRequestDetails['parameters']['order']['currency'];

			$amount = BamboraCurrency::convertPriceFromMinorUnits( $paymentRequestDetails['parameters']['order']['amount'], BamboraCurrency::getCurrencyMinorunits( $currency ) );
			...
			$html .= '<a class="bambora-preview-payment-request btn" href="' . $paymentRequest['payment_request_url'] . '" target="_blank" >' . $this->l( 'Preview' ) . '</a><br/>';



			$merchantPaymentRequestUrl = "https://merchant.bambora.com/".Configuration::get('BAMBORA_MERCHANTNUMBER')."/paymentrequests/". $paymentRequest['payment_request_id'];

			$html .= '<a href="' . $merchantPaymentRequestUrl . '" target="_blank" >' . $this->l( 'Link to Payment Request on Merchant UI' ) . '</a><br/>';
	...
	private function getDbPaymentRequestByCartId( $id_cart ) {
		$query = 'SELECT * FROM ' . _DB_PREFIX_ . 'bambora_payment_requests WHERE id_cart = ' . pSQL( $id_cart );
